...
export default function NavbarSection() {
  ...
  return (
    <header
      className={`fixed inset-x-0 z-[900] flex flex-col backdrop-blur-[6px] transition-all duration-300 ${isScrolled ? 'bg-white/50 backdrop-blur-[10px]' : 'bg-transparent backdrop-blur-none'}`}>
      ...
      <div className="bg-brand-100">
        <div className="container mx-auto flex items-center justify-between !px-0">
          ...
          <Link
            ...
            href="/">
            <div className="absolute start-0 top-0 h-full w-[1000px] -translate-x-[100%] bg-[#0f4074]"></div>
          ...
          <div className="hidden items-center gap-4 pr-4 lg:flex">
            ...
            <Drawer direction="right" open={open} onOpenChange={setOpen}>
              <DrawerTrigger asChild>
                <button
                  className="hover:text-brand-200 cursor-pointer px-3 py-3 outline-none"
                  aria-label="Open menu">
                  <div className="flex h-10 w-10 flex-col items-center justify-center space-y-3">
                    <span
                      className={`block h-[1px] w-[45px] bg-white transition-all duration-300 ${
                        open ? 'translate-y-[14px] rotate-45' : ''
                      }`}
                    />
                    <span
                      className={`block h-[1px] w-[45px] bg-white transition-all duration-300 ${
              ...
              <DrawerContent className="top-0 right-0 z-[910] h-full w-[320px] rounded-none p-6">
                <DrawerClose
                  asChild
                  className="mb-4 border-b-1 border-b-gray-200 pb-2">
                  <button className="flex cursor-pointer items-center justify-end text-gray-950">
                    Close <GrClose className="ml-2" />
                  </button>
                </DrawerClose>
                  ...
                  {mobileMenuData.map((item, index) => {
                    ...
                    return (
                      <div key={`${item.label}-${index}`}>
                        {/* PARENT */}
                        <div className="flex items-center justify-between text-lg text-gray-950">
                          <button
                            onClick={() => {
                              if (!hasChildren) setOpen(false)
                            }}
                            className="group flex-1 cursor-pointer text-left">
                            {item.label}
                        ...
                        {/* CHILDREN (SAME animation as dropdown) */}
                        {hasChildren && (
                          <div
                            ...
                            <div className="flex flex-col gap-2 ps-4 text-start text-gray-800">
                              {item.submenu!.map((subItem, sIdx) => (
                                <button
                                  key={`${subItem}-${sIdx}`}
                                  onClick={() => setOpen(false)}
                                  className="group flex cursor-pointer items-start gap-2 transition hover:text-gray-950">
                                  {subItem}
                                </button>
                              ))}
                            </div>
